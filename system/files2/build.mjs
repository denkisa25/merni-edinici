...
// present, otherwise derive from density. Mirrors calc.js toGrams() for static pages.
// `frac` lets callers ask for a fraction of the unit (e.g. ½ чаша) and still honour the
// exact base value (½ × 220 g rather than re-deriving from ml).
function unitWeight(ing, unitKey, frac = 1) {
  ...
  return null; // mass unit — caller shouldn't ask for grams here
}
...
function computeReferenceRows(ing, liquid) {
  ...
  // dry: exact-first. fractions/multiples scale the exact cup value when we have one.
