...
(function () {
  ...
  /* ----------------------------- data layer ----------------------------- */
  // Each ingredient carries `density` (g/ml — used for liquids and for any unit the
  // Bulgarian source never measured) and optionally `measures`: exact gram/ml weights
  // per Bulgarian unit, copied verbatim from supichka. measures win; density fills gaps.
  var DEFAULT_DATA = {
    ingredients: {
  ...
  // any source quantity → grams.
  // Priority: exact source measure for this unit → else volume×density → else mass unit.
  ...
  // grams → any target unit (same priority order, inverted)
  ...
  function init() {
    ...
    function compute() {
      ...
      if (el.why) {
        // explain the basis: exact source value if we have one for either unit, else density
        var usedMeasure = ing.measures &&
          (ing.measures[el.from.value] != null || ing.measures[el.to.value] != null);
        el.why.textContent =
          pretty(amount) + " " + UNITS[el.from.value].label + " " + ing.name.toLowerCase() +
          " ≈ " + pretty(result) + " " + UNITS[el.to.value].label +
          (usedMeasure ? " · по таблица" : " · плътност " + ing.density.toFixed(2) + " г/мл");
      }
      if (SYNC_URL) {
