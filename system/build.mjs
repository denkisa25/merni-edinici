...
// Units. Volume units carry ml; mass units carry g.
//   slug/label/abbr/gen → used by the generator (URLs + SEO sentences).
//   c (client short label) → used only when emitting data.${lang}.js for the calculator/scaler UI.
// The generator only ever uses chasha/sl/chl as from-units and g as to-unit, so the
// imperial/litre entries below exist purely to feed the shared client dataset.
const UNITS = {
...
// Order the calculator/scaler show units in (emitted into the client dataset).
...
const REV_ANCHOR = 100; // grams, anchor amount for reverse (mass → volume) pages
const UNIT_PAIRS = [
  { from: "chasha", to: "g" },
  { from: "sl",     to: "g" },
  { from: "chl",    to: "g" },
...
const T = {
  bg: {
    ...
    // reverse (mass → volume) page templates: "Колко чаши са 100 грама X?"
    ...
    faq1_a: (ing, n) => `Една супена лъжица (15 мл) ${ing} тежи около ${num(n)} грама.`,
    faq2_q: (ing) => `Колко чаши ${ing} са 500 грама?`,
    faq2_a: (ing, n) => `500 грама ${ing} са приблизително ${num(n)} ${n === 1 ? "чаша" : "чаши"}.`,
...
// Card info for one ingredient×pair — shared by hub cards and question-page slugs so they always match.
...
function computeQuestionPage(ing, pair, lang) {
  ...
  const slug = card.slug, url = card.url;
...
// optional ingredient-specific note + generic density paragraph + cup-ambiguity note
...
// Emits the shared client dataset consumed by calc.js AND the recipe scaler.
// Ingredient densities/names come straight from INGREDIENTS, so the calculator,
// the scaler, and the generated reference tables can never disagree.
...
function build() {
  ...
  // copy shared assets if present, then emit the shared client dataset per language
  if (existsSync("assets")) cpSync("assets", join(SITE.outDir, "assets"), { recursive: true });
  for (const lang of LANGS) write(join(SITE.outDir, "assets", `data.${lang}.js`), renderClientData(lang));

  console.log(`✓ ${count} pages across ${LANGS.length} language(s)`);
  console.log(`✓ sitemap.xml with ${sitemap.length} URLs`);
  console.log(`✓ data.${LANGS.join(".js, data.")}.js (shared dataset for calc.js + scaler)`);
